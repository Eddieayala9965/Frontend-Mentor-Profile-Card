...
async def create_profile(profile: schemas.Profile, db: Session = Depends(database.get_db), current_user: schemas.User = Depends(get_current_user)):
...
async def get_profiles(skip: int = 0, limit: int = 10, db: Session = Depends(database.get_db)):
...
async def update_profile(profile_id: uuid.UUID, profile: schemas.ProfileUpdate, db: Session = Depends(database.get_db), current_user: schemas.User = Depends(get_current_user)):
...
async def delete_profile(profile_id: uuid.UUID, db: Session = Depends(database.get_db), current_user: schemas.User = Depends(get_current_user)):
